...
This is Basic Block Catalog type `84` (`select_const`). It implements logical or event-driven signal behaviour.
...
$$
0 = (yo - (proc_{select\_const\_0} \cdot K_{true} + (1 - proc_{select\_const\_0}) \cdot K_{false}))
$$
...
| Parameter | `K_true` | Configurable model parameter | model-dependent |
| Parameter | `K_false` | Configurable model parameter | model-dependent |
