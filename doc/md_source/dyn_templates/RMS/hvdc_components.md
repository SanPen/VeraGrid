...
- [DC-link capacitor](vsc_dc_link.md), which also owns the DC-terminal power and voltage constraint.
- [DC line](dc_line.md).
...
- Terminal powers use the branch convention: `Pf_vsc`, `Pt_vsc` and `Qt_vsc` are positive into the VSC. The electrical equations expose `Pt_vsc/Qt_vsc` directly; they do not create a second internal `P/Q` pair.
