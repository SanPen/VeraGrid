...
This is Basic Block Catalog type `359` (`LN`). It evaluates the mathematical function identified by the block name.

<!-- veragrid-block-introduction:start -->
**LN** is a mathematical signal-processing primitive. It forms an algebraic relation between inputs, parameters, and outputs and is commonly combined with dynamic and nonlinear blocks to construct larger control equations without introducing an independent physical state.
...
$$
0 = (yo - \ln\left(yi \cdot \Theta\left(yi - 1 \times 10^{-6}\right) + 1 \times 10^{-6}(1 - \Theta\left(yi - 1 \times 10^{-6}\right))\right))
$$
