...
This is Basic Block Catalog type `507` (`Switch par 1->1 by par`). It implements logical or event-driven signal behaviour.
...
$$
0 = (yo - (proc_{select\_0} \cdot yi + (1 - proc_{select\_0}) \cdot p))
$$
...
| Parameter | `Enable` | Configurable model parameter | model-dependent |
