...
This is Basic Block Catalog type `512` (`Switch sig 1->1 by sig (fixed)`). It implements logical or event-driven signal behaviour.
...
$$
0 = (yo - (proc_{selfix\_0} \cdot yi + (1 - proc_{selfix\_0}) \cdot p))
$$
...
| Parameter | `p` | Configurable model parameter | model-dependent |
