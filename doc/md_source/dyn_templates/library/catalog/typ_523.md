...
This is Basic Block Catalog type `523` (`Switch sig 3->1 by sig`). It implements logical or event-driven signal behaviour.
...
$$
0 = (yo - (proc_{select\_1} \cdot yi1 + (1 - proc_{select\_1}) \cdot (proc_{select\_0} \cdot yi2 + (1 - proc_{select\_0}) \cdot yi3)))
$$
