...
This is Basic Block Catalog type `530` (`Switch sw smaller than or equal C 2s->1s`). It implements logical or event-driven signal behaviour.
...
$$
0 = (yo - (proc_{select\_0} \cdot yi1 + (1 - proc_{select\_0}) \cdot yi2))
$$
...
| Input | `sw` | Input signal consumed by the block | model-dependent |
