...
This is Basic Block Catalog type `8` (`Lookup array (linear_noclipping)`). It evaluates tabulated or array-based data using the selected lookup formulation.
...
$$
0 = (yo - ((\frac{arr_{y2} - arr_{y1}}{arr_{x2} - arr_{x1}} \cdot yi + arr_{y1} - \frac{arr_{y2} - arr_{y1}}{arr_{x2} - arr_{x1}} \cdot arr_{x1}) \cdot \Theta\left(arr_{x2} - yi - 1 \times 10^{-6}\right) + (\frac{arr_{y3} - arr_{y2}}{arr_{x3} - arr_{x2}} \cdot yi + arr_{y2} - \frac{arr_{y3} - arr_{y2}}{arr_{x3} - arr_{x2}} \cdot arr_{x2}) \cdot \Theta\left(yi - arr_{x2} + 1 \times 10^{-6}\right)))
$$
...
| Parameter | `array_K` | Configurable model parameter | model-dependent |
