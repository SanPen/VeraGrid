...
class DgsExportDialogue(QtWidgets.QDialog):
    """
    Dialogue to export the active circuit to PowerFactory DGS.
    """

    def __init__(self, app: IoMain) -> None:
        """
        Build the DGS export dialogue.
    ...
    def _populate_static_controls(self) -> None:
        ...
        :return: None.
        """
        for export_mode in self.dgs_export_modes:
            self.ui.dgs_export_mode_comboBox.addItem(str(export_mode))
    ...
    def _connect_signals(self) -> None:
        ...
        self.ui.dgs_export_mode_comboBox.currentIndexChanged.connect(self.update_mode_dependent_widgets)

    def get_selected_export_mode(self) -> DgsExportMode:
        """
        Get the selected DGS export strategy.

        :return: Selected DGS export mode.
        """
        combo_index: int = self.ui.dgs_export_mode_comboBox.currentIndex()
        return self.dgs_export_modes[combo_index]
    ...
    def update_mode_dependent_widgets(self) -> None:
        ...
        :return: None.
        """
        export_mode: DgsExportMode = self.get_selected_export_mode()
        ...
        # operation summary.
        if export_mode == DgsExportMode.BatchZip:
            self.ui.time_slot_slider.setEnabled(False)
            if has_time_series:
    ...
    def get_dialogue_file_filter(self, export_mode: DgsExportMode) -> str:
        """
        ...
        if export_mode == DgsExportMode.BatchZip:
            ...
            return "Power Factory (*.dgs)"

    def normalize_export_file_name(self,
                                   file_name: str,
                                   export_mode: DgsExportMode) -> str:
        """
        ...
        if export_mode == DgsExportMode.BatchZip:
            ...
            if normalized_extension == ".dgs":
                return file_name
            else:
                return root_name + ".dgs"

    def export(self) -> None:
        """
        Collect the dialogue settings and launch the DGS export.

        :return: None.
        """
        export_mode: DgsExportMode = self.get_selected_export_mode()

        if export_mode == DgsExportMode.BatchZip and not self.app.circuit.has_time_series:
        ...
        if selected_file_name == '':
            ...
            self.app.save_file_now(
                ...
                options=filedrv.FileSavingOptions(
                    ...
                    dgs_export_mode=export_mode,
                    t_idx=selected_t_idx
                )
            )

            if export_mode == DgsExportMode.BatchZip:
                self.app.show_info_toast("DGS batch export started")
            else:
                self.app.show_info_toast("DGS export started")
