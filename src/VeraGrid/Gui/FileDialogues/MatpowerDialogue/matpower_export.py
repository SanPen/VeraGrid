...
from VeraGrid.Gui.FileDialogues.MatpowerDialogue.matpower_export_gui import Ui_MatpowerExportDialog
from VeraGridEngine.enumerations import FileType, MatpowerExportMode

if TYPE_CHECKING:
    from VeraGrid.Gui.Main.SubClasses.io import IoMain


class MatpowerExportDialogue(QtWidgets.QDialog):
    """
    Dialogue to export the active circuit to MATPOWER.
    """

    def __init__(self, app: IoMain) -> None:
        """
        Build the MATPOWER export dialogue.
        ...
        self.matpower_export_modes: list[MatpowerExportMode] = [
            MatpowerExportMode.SingleFile,
            MatpowerExportMode.BatchZip,
        ]
    ...
    def _populate_static_controls(self) -> None:
        ...
        :return: None.
        """
        export_mode: MatpowerExportMode
        for export_mode in self.matpower_export_modes:
            self.ui.matpower_export_mode_comboBox.addItem(str(export_mode))
    ...
    def _connect_signals(self) -> None:
        ...
        self.ui.matpower_export_mode_comboBox.currentIndexChanged.connect(self.update_mode_dependent_widgets)

    def get_selected_export_mode(self) -> MatpowerExportMode:
        """
        Get the selected MATPOWER export strategy.

        :return: Selected MATPOWER export mode.
        """
        combo_index: int = self.ui.matpower_export_mode_comboBox.currentIndex()
        return self.matpower_export_modes[combo_index]
    ...
    def update_mode_dependent_widgets(self) -> None:
        ...
        :return: None.
        """
        export_mode: MatpowerExportMode = self.get_selected_export_mode()
        has_time_series: bool = self.app.circuit.has_time_series

        if export_mode == MatpowerExportMode.BatchZip:
            self.ui.time_slot_slider.setEnabled(False)
            if has_time_series:
    ...
    def get_dialogue_file_filter(self, export_mode: MatpowerExportMode) -> str:
        """
        ...
        if export_mode == MatpowerExportMode.BatchZip:
            ...
            return "MATPOWER (*.m *.matpower)"

    def normalize_export_file_name(self,
                                   file_name: str,
                                   export_mode: MatpowerExportMode) -> str:
        """
        ...
        if export_mode == MatpowerExportMode.BatchZip:
            ...
            if normalized_extension in [".m", ".matpower"]:
                return file_name
            else:
                return root_name + ".m"
    ...
    def export(self) -> None:
        """
        Collect the dialogue settings and launch the MATPOWER export.

        :return: None.
        """
        export_mode: MatpowerExportMode = self.get_selected_export_mode()

        if export_mode == MatpowerExportMode.BatchZip and not self.app.circuit.has_time_series:
        ...
        if selected_file_name == "":
            ...
            self.app.save_file_now(
                ...
                options=filedrv.FileSavingOptions(
                    file_type=FileType.Matpower,
                    matpower_export_mode=export_mode,
                    t_idx=selected_t_idx,
                )
            )

            if export_mode == MatpowerExportMode.BatchZip:
                self.app.show_info_toast("MATPOWER batch export started")
            else:
                self.app.show_info_toast("MATPOWER export started")
