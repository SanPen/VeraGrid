...
class ProfileBool:
    ...
    def __init__(
        ...
        self._sparse_array: SparseArrayBool | None = None
        self._dense_array: np.ndarray | None = None
        self._sparsity_threshold: float = sparsity_threshold
        self._dtype: type[bool] = bool
        self._initialized: bool = False
        self._default_value: bool = coerce_bool_value(default_value)
    ...
    def get_sparse_map(self) -> dict[int, bool]:
        """
    ...
    def default_value(self, val: bool) -> None:
        """
        ...
        :return: ``None``.
        """
        self._default_value = coerce_bool_value(val)
    ...
    @property
    def sparse_array(self) -> SparseArrayBool | None:
        """
    ...
    def create_sparse(self, size: int, default_value: bool, map_data: dict[int, bool] | None = None) -> None:
        """
        ...
        self._default_value = coerce_bool_value(default_value)
        self._sparse_array = SparseArrayBool(default_value=self._default_value)
    ...
    def create_dense(self, size: int, default_value: bool) -> None:
        """
        ...
        self._is_sparse = False
        self._default_value = coerce_bool_value(default_value)
        self._dense_array = np.full(size, self._default_value, dtype=bool)
        self._sparse_array = None
        self._initialized = True
        ...
        if not isinstance(arr, np.ndarray):
            ...
            if arr.ndim != 1:
                ...
                if len(arr) == 0:
                    ...
                    try:
                        arr_mod: np.ndarray = np.asarray(arr, dtype=bool)
                    ...
                    counts: Counter[bool] = Counter(arr_mod.tolist())
                    most_common_element: bool
                    ...
                    if sparsity_factor >= self._sparsity_threshold:
                        base_value: bool = coerce_bool_value(most_common_element)
                        data_map: dict[int, bool] = dict()
                        non_default_indices: np.ndarray = np.flatnonzero(arr_mod != base_value)

                        # The boolean path keeps sparse compression vectorized on large arrays.
                        for raw_idx in non_default_indices:
                            idx: int = int(raw_idx)
                            data_map[idx] = bool(arr_mod[idx])

                        self._is_sparse = True
                        self._default_value = base_value
                        self._sparse_array = SparseArrayBool(default_value=base_value)
                        ...
                        self._sparse_array = None
                        self._default_value = bool(arr_mod[0])
    ...
    def __eq__(self, other: object) -> bool:
        ...
        """
        if isinstance(other, ProfileBool):
    ...
    def __getitem__(self, key: int) -> bool:
        ...
        if self._is_sparse:
            ...
            if self._dense_array is None:
                self._is_sparse = True
                self._sparse_array = SparseArrayBool(default_value=self.default_value)
                print("Initializing sparse when querying, this signals a mis initialization")
                return self.default_value
            else:
                return bool(self._dense_array[key])
    ...
    def __setitem__(self, key: int, value: bool) -> None:
        ...
        if isinstance(key, int):
            coerced_value: bool = coerce_bool_value(value)
    ...
    def __imul__(self, other: float | int) -> "ProfileBool":
        """
        Reject in-place scaling because boolean profiles are not numeric.
        ...
        if isinstance(n, int):
            if self._initialized:
                if self._is_sparse:
                    ...
                    if self._dense_array is not None:
                        new_array: np.ndarray = np.full(n, self.default_value, dtype=bool)
    ...
    def fill(self, value: bool) -> None:
        ...
        if self._sparse_array is None:
            self._sparse_array = SparseArrayBool(default_value=coerced_value)
        else:
            self._sparse_array = self._sparse_array
    ...
    def get_sparse_representation(self) -> tuple[list[int], list[bool]]:
        """
    ...
    def set_sparse_data_from_data(self, indptr: list[int], data: list[bool]) -> None:
        """
    ...
    def copy(self) -> "ProfileBool":
        """
        Build a deep copy of the profile.

        :return: New profile copy.
        """
        copied_profile: ProfileBool = ProfileBool(
            default_value=self.default_value,
            arr=None,
            sparsity_threshold=self._sparsity_threshold,
