...
class ProfileEnum:
    ...
    def __init__(
        ...
        """
        Build an enum profile.

        :param default_value: Declared default value.
        :param enum_type: Enum class stored by the profile.
        ...
        self._sparse_array: SparseArrayEnum | None = None
        self._dense_array: np.ndarray | None = None
        self._sparsity_threshold: float = sparsity_threshold
        self._dtype: type[Enum] = enum_type
        self._initialized: bool = False
        self._default_value: Enum | None = self._coerce_value(default_value)
    ...
    def _coerce_value(self, value: Any) -> Enum | None:
        """
        Coerce a raw value into the declared enum type.
    ...
    def get_sparse_map(self) -> dict[int, Enum | None]:
        """
    ...
    @property
    def default_value(self) -> Enum | None:
        """
    ...
    def default_value(self, val: Enum | None) -> None:
        """
    ...
    @property
    def sparse_array(self) -> SparseArrayEnum | None:
        """
    ...
    def create_sparse(self, size: int, default_value: Enum | None, map_data: dict[int, Enum | None] | None = None) -> None:
        """
        ...
        self._sparse_array = SparseArrayEnum(default_value=self._default_value, enum_type=self._dtype)
    ...
    def create_dense(self, size: int, default_value: Enum | None) -> None:
        """
        ...
        if not isinstance(arr, np.ndarray):
            ...
            if arr.ndim != 1:
                ...
                if len(arr) == 0:
                    ...
                    arr_values: list[Enum | None] = list()

                    # Coerce every element explicitly so the profile never keeps mixed enum payloads.
                    ...
                    counts: Counter[Enum | None] = Counter(arr_mod.tolist())
                    most_common_element: Enum | None
                    ...
                    if sparsity_factor >= self._sparsity_threshold:
                        base_value: Enum | None = self._coerce_value(most_common_element)
                        data_map: dict[int, Enum | None] = dict()

                        for idx, raw_value in enumerate(arr_mod):
                            coerced_value: Enum | None = self._coerce_value(raw_value)
                        ...
                        self._sparse_array = SparseArrayEnum(default_value=base_value, enum_type=self._dtype)
    ...
    def __eq__(self, other: object) -> bool:
        ...
        """
        if isinstance(other, ProfileEnum):
    ...
    def __getitem__(self, key: int) -> Enum | None:
        ...
        if self._is_sparse:
            ...
            if self._dense_array is None:
                self._is_sparse = True
                self._sparse_array = SparseArrayEnum(default_value=self.default_value, enum_type=self._dtype)
                print("Initializing sparse when querying, this signals a mis initialization")
                return self.default_value
            else:
                return self._coerce_value(self._dense_array[key])

    def __setitem__(self, key: int, value: Enum | None) -> None:
        """
        ...
        if isinstance(key, int):
            coerced_value: Enum | None = self._coerce_value(value)
    ...
    def __imul__(self, other: float | int) -> "ProfileEnum":
        """
        Reject in-place scaling because enum profiles are not numeric.
    ...
    def fill(self, value: Enum | None) -> None:
        """
        ...
        if self._sparse_array is None:
            self._sparse_array = SparseArrayEnum(default_value=coerced_value, enum_type=self._dtype)
        else:
            self._sparse_array = self._sparse_array
    ...
    def tolist(self) -> list[Enum | None]:
        """
    ...
    def get_sparse_representation(self) -> tuple[list[int], list[Enum | None]]:
        """
    ...
    def set_sparse_data_from_data(self, indptr: list[int], data: list[Enum | None]) -> None:
        """
    ...
    def copy(self) -> "ProfileEnum":
        """
        Build a deep copy of the profile.

        :return: New profile copy.
        """
        copied_profile: ProfileEnum = ProfileEnum(
            default_value=self.default_value,
            enum_type=self._dtype,
            arr=None,
            sparsity_threshold=self._sparsity_threshold,
