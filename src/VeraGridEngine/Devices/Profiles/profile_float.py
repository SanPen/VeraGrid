...
class ProfileFloat:
    ...
    def __init__(
        ...
        self._sparse_array: SparseArrayFloat | None = None
        self._dense_array: np.ndarray | None = None
        self._sparsity_threshold: float = sparsity_threshold
        self._dtype: type[float] = float
        self._initialized: bool = False
        self._default_value: float = float(default_value)
    ...
    def get_sparse_map(self) -> dict[int, float]:
        """
    ...
    def default_value(self, val: float) -> None:
        """
    ...
    @property
    def sparse_array(self) -> SparseArrayFloat | None:
        """
    ...
    def create_sparse(self, size: int, default_value: float, map_data: dict[int, float] | None = None) -> None:
        """
        ...
        self._sparse_array = SparseArrayFloat(default_value=self._default_value)
    ...
    def create_dense(self, size: int, default_value: float) -> None:
        """
        ...
        self._is_sparse = False
        self._default_value = float(default_value)
        self._dense_array = np.full(size, self._default_value, dtype=float)
        self._sparse_array = None
        self._initialized = True
        ...
        if not isinstance(arr, np.ndarray):
            ...
            if arr.ndim != 1:
                ...
                if len(arr) == 0:
                    ...
                    try:
                        arr_mod: np.ndarray = np.asarray(arr, dtype=float)
                    ...
                    counts: Counter[float] = Counter(arr_mod.tolist())
                    most_common_element: float
                    ...
                    if sparsity_factor >= self._sparsity_threshold:
                        base_value: float = float(most_common_element)
                        data_map: dict[int, float] = dict()
                        ...
                        for raw_idx in non_default_indices:
                            idx: int = int(raw_idx)
                            data_map[idx] = float(arr_mod[idx])

                        self._is_sparse = True
                        self._default_value = base_value
                        self._sparse_array = SparseArrayFloat(default_value=base_value)
                        ...
                        self._sparse_array = None
                        self._default_value = float(arr_mod[0])
    ...
    def __eq__(self, other: object) -> bool:
        ...
        """
        if isinstance(other, ProfileFloat):
                ...
                if self._is_sparse:
                    ...
                    if self._dense_array is None and other._dense_array is None:
                        ...
                        if self._dense_array is not None and other._dense_array is not None:
                            return np.allclose(self._dense_array, other._dense_array, atol=1.0e-10)
                        else:
                            return False
            else:
                return False
    ...
    def __getitem__(self, key: int) -> float:
        ...
        if self._is_sparse:
            ...
            if self._dense_array is None:
                self._is_sparse = True
                self._sparse_array = SparseArrayFloat(default_value=self.default_value)
                print("Initializing sparse when querying, this signals a mis initialization")
                return self.default_value
            else:
                return float(self._dense_array[key])
    ...
    def __setitem__(self, key: int, value: float) -> None:
        ...
        if isinstance(key, int):
            coerced_value: float = float(value)
    ...
    def __imul__(self, other: float | int) -> "ProfileFloat":
        """
        ...
        if isinstance(n, int):
            if self._initialized:
                if self._is_sparse:
                    ...
                    if self._dense_array is not None:
                        new_array: np.ndarray = np.full(n, self.default_value, dtype=float)
    ...
    def fill(self, value: float) -> None:
        ...
        if self._sparse_array is None:
            self._sparse_array = SparseArrayFloat(default_value=coerced_value)
        else:
            self._sparse_array = self._sparse_array
    ...
    def scale(self, value: float | int) -> None:
        ...
        if self._is_sparse:
            if self._sparse_array is not None:
                ...
                new_default_value: float = float(self.default_value * value)
                new_map: dict[int, float] = dict()
                ...
                for key, sparse_value in self._sparse_array.get_map().items():
                    scaled_value: float = float(sparse_value * value)
                    if scaled_value != new_default_value:
                        new_map[key] = scaled_value
                    else:
                        new_map = new_map

                self._sparse_array = SparseArrayFloat(default_value=new_default_value)
            ...
            if self._dense_array is not None:
                ...
                self._default_value = float(self.default_value * value)
            else:
                raise AssertionError("Dense profile not initialized")
    ...
    def get_sparse_representation(self) -> tuple[list[int], list[float]]:
        """
    ...
    def set_sparse_data_from_data(self, indptr: list[int], data: list[float]) -> None:
        """
    ...
    def fix_nan(self, default_value: float = 0.0) -> None:
        ...
        :param default_value: Replacement value.
        :return: ``None``.
        """
        if not self._is_sparse:
            if self._dense_array is not None:
                np.nan_to_num(self._dense_array, nan=default_value, copy=False)
            else:
                self._dense_array = self._dense_array
        else:
            self._is_sparse = self._is_sparse

    def copy(self) -> "ProfileFloat":
        """
        Build a deep copy of the profile.

        :return: New profile copy.
        """
        copied_profile: ProfileFloat = ProfileFloat(
            default_value=self.default_value,
            arr=None,
            sparsity_threshold=self._sparsity_threshold,
