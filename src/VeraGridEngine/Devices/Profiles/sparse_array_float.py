...
class SparseArrayFloat:
    ...
    def __init__(self, default_value: float, size: int = 0) -> None:
        """
        Build a float sparse array.
        ...
        self._size: int = size
        self._map: dict[int, float] = dict()

    def copy(self) -> "SparseArrayFloat":
        """
        Build a deep copy of the sparse array.

        :return: New sparse array copy.
        """
        copied_array: SparseArrayFloat = SparseArrayFloat(default_value=self._default_value, size=self._size)
    ...
    def create(self, size: int, default_value: float, data: dict[int, float] | None = None) -> "SparseArrayFloat":
        """
    ...
    def create_from_array(self, array: np.ndarray, default_value: float) -> "SparseArrayFloat":
        """
        Create the sparse array from a dense array.

        :param array: Dense float array.
        :param default_value: Default sparse value.
        :return: ``self``.
        """
        dense_array: np.ndarray = np.asarray(array, dtype=float)
        self.default_value = default_value
        self._size = len(dense_array)
        self._map = dict()

        # The dense float path uses vectorized comparison to identify the sparse coordinates quickly.
        non_default_indices: np.ndarray = np.flatnonzero(dense_array != self._default_value)

        for raw_idx in non_default_indices:
            idx: int = int(raw_idx)
            self._map[idx] = float(dense_array[idx])

        return self

    def create_from_dict(self, default_value: float, size: int, map_data: dict[int, float]) -> "SparseArrayFloat":
        """
    ...
    def toarray(self) -> np.ndarray:
        ...
        """
        dense_array: np.ndarray = np.full(self._size, self._default_value, dtype=float)
    ...
    def at(self, idx: int) -> float:
        ...
        if len(self._map) == 0:
            ...
            value: float | None = self._map.get(idx, None)
            if value is None:
                return self._default_value
            else:
                return value
    ...
    def __eq__(self, other: object) -> bool:
        ...
        """
        if isinstance(other, SparseArrayFloat):
    ...
    def set_data(self, d: dict[int, float]) -> None:
        """
    ...
    def resample(self, indices: IntVec) -> None:
        ...
        for new_idx, old_idx in enumerate(indices):
            sparse_value: float | None = self._map.get(int(old_idx), None)
    ...
    def slice(self, indices: IntVec) -> "SparseArrayFloat":
        """
        ...
        """
        sliced_array: SparseArrayFloat = self.copy()
