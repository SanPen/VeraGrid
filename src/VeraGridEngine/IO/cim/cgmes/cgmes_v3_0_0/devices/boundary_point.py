...
The ISO code is a two-character country code as defined by ISO 3166 (http://www.iso.org/iso/country_codes). The length of the string is 2 characters maximum.''', mandatory=True, profiles=[CgmesProfileType.EQ, CgmesProfileType.EQ_BD]),
...
-if the Boundary point is placed in a substation, it is the name (IdentifiedObject.name) of the element (e.g. PowerTransformer, ACLineSegment, Switch, etc.) at which the "From" side of the Boundary point is connected to.''', mandatory=True, profiles=[CgmesProfileType.EQ, CgmesProfileType.EQ_BD]),
		CgmesProperty(property_name='fromEndNameTso', class_type=str, multiplier=UnitMultiplier.none, unit=UnitSymbol.none, description='''Identifies the name of the transmission system operator, distribution system operator or other entity at which the "From" side of the interconnection is connected to. The length of the string is 64 characters maximum.''', mandatory=True, profiles=[CgmesProfileType.EQ, CgmesProfileType.EQ_BD]),
...
-if the Boundary point is placed in a substation, it is the name (IdentifiedObject.name) of the element (e.g. PowerTransformer, ACLineSegment, Switch, etc.) at which the "To" side of the Boundary point is connected to.''', mandatory=True, profiles=[CgmesProfileType.EQ, CgmesProfileType.EQ_BD]),
		...
		CgmesProperty(property_name='isDirectCurrent', class_type=bool, multiplier=UnitMultiplier.none, unit=UnitSymbol.none, description='''If true, this boundary point is a point of common coupling (PCC) of a direct current (DC) interconnection, otherwise the interconnection is AC (default).''', profiles=[CgmesProfileType.EQ, CgmesProfileType.EQ_BD]),
		CgmesProperty(property_name='isExcludedFromAreaInterchange', class_type=bool, multiplier=UnitMultiplier.none, unit=UnitSymbol.none, description='''If true, this boundary point is on the interconnection that is excluded from control area interchange calculation and consequently has no related tie flows. Otherwise, the interconnection is included in control area interchange and a TieFlow is required at all sides of the boundary point (default).''', profiles=[CgmesProfileType.EQ, CgmesProfileType.EQ_BD]),
		CgmesProperty(property_name='ConnectivityNode', class_type='ConnectivityNode', multiplier=UnitMultiplier.none, unit=UnitSymbol.none, description='''The connectivity node that is designated as a boundary point.''', mandatory=True, profiles=[CgmesProfileType.EQ, CgmesProfileType.EQ_BD]),
	)
