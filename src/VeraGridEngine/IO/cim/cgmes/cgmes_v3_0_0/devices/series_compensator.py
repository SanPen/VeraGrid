...
class SeriesCompensator(ConductingEquipment):
	LOCAL_CGMES_PROPERTIES: tuple[CgmesProperty, ...] = (
		...
		CgmesProperty(property_name='varistorPresent', class_type=bool, multiplier=UnitMultiplier.none, unit=UnitSymbol.none, description='''Describe if a metal oxide varistor (mov) for over voltage protection is configured in parallel with the series compensator. It is used for short circuit calculations.''', mandatory=True, profiles=[CgmesProfileType.SC]),
		CgmesProperty(property_name='varistorRatedCurrent', class_type=float, multiplier=UnitMultiplier.none, unit=UnitSymbol.A, description='''Electrical current with sign convention: positive flow is out of the conducting equipment into the connectivity node. Can be both AC and DC.''', profiles=[CgmesProfileType.SC]),
		CgmesProperty(property_name='varistorVoltageThreshold', class_type=float, multiplier=UnitMultiplier.k, unit=UnitSymbol.V, description='''Electrical voltage, can be both AC and DC.''', profiles=[CgmesProfileType.SC]),
	)
	__slots__ = ('r', 'x', 'r0', 'x0', 'varistorPresent', 'varistorRatedCurrent', 'varistorVoltageThreshold')
