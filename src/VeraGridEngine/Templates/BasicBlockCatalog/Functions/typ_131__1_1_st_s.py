...
def build_typ_131__1_1_st_s_default_template_name() -> str:
    """
    Return the canonical runtime name for this standalone template.

    :returns: Default template name.
    """
    return '1/(1+sT) (s)__131'

def build_typ_131__1_1_st_s_template(vf: VarFactory, name: str | None = None) -> EmtModelTemplate:
    """
    ...
    template_name: str
    if name is None:
        template_name = build_typ_131__1_1_st_s_default_template_name()
    else:
        template_name = name
    ...
    v_1_1_sT_s_T: Var = vf.add_var('1/(1+sT) (s)__T_' + template_name)
    # Declare the state variables used by the template.
    v_1_1_sT_s_x: Var = vf.add_var('1/(1+sT) (s)__x_' + template_name)
    ...
    d_1_1_sT_s_x: Var = vf.add_diff_var('d_1/(1+sT) (s)__x_' + template_name, base_var=v_1_1_sT_s_x)

    # Build explicit typed collections so the symbolic surface is easy to inspect.
    state_equations: list[Expr] = list()
    state_equations.append(((yi - v_1_1_sT_s_x) / v_1_1_sT_s_T))
    state_variables: list[Var] = list()
    state_variables.append(v_1_1_sT_s_x)
    algebraic_equations: list[Expr] = list()
    algebraic_equations.append((yo - (((-y_max) + ((v_1_1_sT_s_x - (-y_max)) * sym.heaviside((v_1_1_sT_s_x - (-y_max))))) - ((v_1_1_sT_s_x - y_max) * sym.heaviside((v_1_1_sT_s_x - y_max))))))
