...
def build_typ_284__1_st_default_template_name() -> str:
    """
...
def build_typ_284__1_st_template(vf: VarFactory, name: str | None = None) -> EmtModelTemplate:
    """
    ...
    template_name: str
    if name is None:
        template_name = build_typ_284__1_st_default_template_name()
    else:
        template_name = name
    ...
    v_1_sT_proc_selfix_0: Var = vf.add_var('1/sT__proc_selfix_0_' + template_name)
    ...
    state_equations.append(((v_1_sT_proc_selfix_0 * (yi / v_1_sT_T)) + ((sym.Const(1.0) - v_1_sT_proc_selfix_0) * sym.Const(0.0))))
    state_variables: list[Var] = list()
    state_variables.append(v_1_sT_x)
    algebraic_equations: list[Expr] = list()
    algebraic_equations.append((yo - (((-sym.Const(999999999999.0)) + ((v_1_sT_x - (-sym.Const(999999999999.0))) * sym.heaviside((v_1_sT_x - (-sym.Const(999999999999.0)))))) - ((v_1_sT_x - v_1_sT_y_max) * sym.heaviside((v_1_sT_x - v_1_sT_y_max))))))
    ...
    mode_parameters[v_1_sT_proc_selfix_0] = vf.add_const(0.0, name='')
    initial_equations: dict[Var, Expr | Const] = dict()
    differential_initial_equations: dict[Var, Expr | Const] = dict()
    procedural_logic_entries: list[object] = list()
    procedural_logic_entries.append(selfix(sym.Comparison(lhs=v_1_sT_T, op=sym.CmpOp.GT, rhs=0.0), output=v_1_sT_proc_selfix_0))
    procedural_logic_entries.append(reset(v_1_sT_x, rst, xrst))
