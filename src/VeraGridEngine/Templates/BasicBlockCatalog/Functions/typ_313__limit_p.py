...
def build_typ_313__limit_p_default_template_name() -> str:
    """
...
def build_typ_313__limit_p_template(vf: VarFactory, name: str | None = None) -> EmtModelTemplate:
    """
    ...
    template_name: str
    if name is None:
        template_name = build_typ_313__limit_p_default_template_name()
    else:
        template_name = name
    ...
    Limit_p_proc_selfix_0: Var = vf.add_var('Limit [p__proc_selfix_0_' + template_name)
    ...
    algebraic_equations.append((yo - ((Limit_p_proc_selfix_0 * Limit_p_y_max) + ((sym.Const(1.0) - Limit_p_proc_selfix_0) * ((Limit_p_y_min + ((yi - Limit_p_y_min) * sym.heaviside((yi - Limit_p_y_min)))) - ((yi - Limit_p_y_max) * sym.heaviside((yi - Limit_p_y_max))))))))
    ...
    mode_parameters[Limit_p_proc_selfix_0] = vf.add_const(0.0, name='')
    initial_equations: dict[Var, Expr | Const] = dict()
    differential_initial_equations: dict[Var, Expr | Const] = dict()
    procedural_logic_entries: list[object] = list()
    procedural_logic_entries.append(selfix(sym.Comparison(lhs=Limit_p_y_max, op=sym.CmpOp.LE, rhs=Limit_p_y_min), output=Limit_p_proc_selfix_0))
