...
def build_typ_317__limit_p_default_template_name() -> str:
    """
    Return the canonical runtime name for this standalone template.

    :returns: Default template name.
    """
    return 'Limit [p]__317'

def build_typ_317__limit_p_template(vf: VarFactory, name: str | None = None) -> EmtModelTemplate:
    """
    ...
    template_name: str
    if name is None:
        template_name = build_typ_317__limit_p_default_template_name()
    else:
        template_name = name
    ...
    Limit_p_y_lim: Var = vf.add_var('Limit [p]__y_lim_' + template_name)
    ...
    algebraic_equations.append((yo - (((-Limit_p_y_lim) + ((yi - (-Limit_p_y_lim)) * sym.heaviside((yi - (-Limit_p_y_lim))))) - ((yi - Limit_p_y_lim) * sym.heaviside((yi - Limit_p_y_lim))))))
    ...
    event_parameters[Limit_p_y_lim] = vf.add_const(None, name='y_lim')
