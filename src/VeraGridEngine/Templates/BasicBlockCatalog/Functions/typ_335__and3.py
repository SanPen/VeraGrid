...
"""
Standalone EMT template for the basic catalog block 'AND3'.
...
def build_typ_335__and3_default_template_name() -> str:
    """
...
def build_typ_335__and3_template(vf: VarFactory, name: str | None = None) -> EmtModelTemplate:
    """
    ...
    template_name: str
    if name is None:
        template_name = build_typ_335__and3_default_template_name()
    else:
        template_name = name
    ...
    AND3_proc_select_const_0: Var = vf.add_var('AND3__proc_select_const_0_' + template_name)
    ...
    algebraic_equations.append((yo - ((AND3_proc_select_const_0 * sym.Const(1.0)) + ((sym.Const(1.0) - AND3_proc_select_const_0) * sym.Const(0.0)))))
    ...
    mode_parameters[AND3_proc_select_const_0] = vf.add_const(0.0, name='')
    initial_equations: dict[Var, Expr | Const] = dict()
    differential_initial_equations: dict[Var, Expr | Const] = dict()
    procedural_logic_entries: list[object] = list()
    procedural_logic_entries.append(sampled_value(output=AND3_proc_select_const_0, source=((yi1 * yi2) * yi3)))
