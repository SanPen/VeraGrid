...
"""
Standalone EMT template for the basic catalog block 'AND4'.
...
def build_typ_336__and4_default_template_name() -> str:
    """
...
def build_typ_336__and4_template(vf: VarFactory, name: str | None = None) -> EmtModelTemplate:
    """
    ...
    template_name: str
    if name is None:
        template_name = build_typ_336__and4_default_template_name()
    else:
        template_name = name
    ...
    AND4_proc_select_const_0: Var = vf.add_var('AND4__proc_select_const_0_' + template_name)
    ...
    algebraic_equations.append((yo - ((AND4_proc_select_const_0 * sym.Const(1.0)) + ((sym.Const(1.0) - AND4_proc_select_const_0) * sym.Const(0.0)))))
    ...
    mode_parameters[AND4_proc_select_const_0] = vf.add_const(0.0, name='')
    initial_equations: dict[Var, Expr | Const] = dict()
    differential_initial_equations: dict[Var, Expr | Const] = dict()
    procedural_logic_entries: list[object] = list()
    procedural_logic_entries.append(sampled_value(output=AND4_proc_select_const_0, source=(((yi1 * yi2) * yi3) * yi4)))
