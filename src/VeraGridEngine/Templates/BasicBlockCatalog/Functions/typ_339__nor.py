...
"""
Standalone EMT template for the basic catalog block 'NOR'.
...
def build_typ_339__nor_default_template_name() -> str:
    """
...
def build_typ_339__nor_template(vf: VarFactory, name: str | None = None) -> EmtModelTemplate:
    """
    ...
    template_name: str
    if name is None:
        template_name = build_typ_339__nor_default_template_name()
    else:
        template_name = name
    ...
    NOR_proc_select_0: Var = vf.add_var('NOR__proc_select_0_' + template_name)
    ...
    algebraic_equations.append((yo - ((NOR_proc_select_0 * sym.Const(0.0)) + ((sym.Const(1.0) - NOR_proc_select_0) * sym.Const(1.0)))))
    ...
    mode_parameters[NOR_proc_select_0] = vf.add_const(0.0, name='')
    initial_equations: dict[Var, Expr | Const] = dict()
    differential_initial_equations: dict[Var, Expr | Const] = dict()
    procedural_logic_entries: list[object] = list()
    procedural_logic_entries.append(sampled_value(output=NOR_proc_select_0, source=(1.0 - ((1.0 - yi1) * (1.0 - yi2)))))
