...
"""
Standalone EMT template for the basic catalog block 'LOG'.
...
def build_typ_360__log_default_template_name() -> str:
    """
...
def build_typ_360__log_template(vf: VarFactory, name: str | None = None) -> EmtModelTemplate:
    """
    ...
    template_name: str
    if name is None:
        template_name = build_typ_360__log_default_template_name()
    else:
        template_name = name
    ...
    algebraic_equations.append((yo - sym.log10(((yi * sym.heaviside((yi - sym.Const(1e-06)))) + (sym.Const(1e-06) * (sym.Const(1) - sym.heaviside((yi - sym.Const(1e-06)))))))))
