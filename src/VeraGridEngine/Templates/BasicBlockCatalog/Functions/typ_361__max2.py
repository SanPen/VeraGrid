...
"""
Standalone EMT template for the basic catalog block 'MAX2'.
...
def build_typ_361__max2_default_template_name() -> str:
    """
...
def build_typ_361__max2_template(vf: VarFactory, name: str | None = None) -> EmtModelTemplate:
    """
    ...
    template_name: str
    if name is None:
        template_name = build_typ_361__max2_default_template_name()
    else:
        template_name = name
    ...
    algebraic_equations.append((yo - ((yi1 * sym.heaviside((yi1 - yi2))) + (yi2 * (sym.Const(1) - sym.heaviside((yi1 - yi2)))))))
