...
"""
Standalone EMT template for the basic catalog block 'MIN2'.
...
def build_typ_364__min2_default_template_name() -> str:
    """
...
def build_typ_364__min2_template(vf: VarFactory, name: str | None = None) -> EmtModelTemplate:
    """
    ...
    template_name: str
    if name is None:
        template_name = build_typ_364__min2_default_template_name()
    else:
        template_name = name
    ...
    algebraic_equations.append((yo - ((yi1 * sym.heaviside((yi2 - yi1))) + (yi2 * (sym.Const(1) - sym.heaviside((yi2 - yi1)))))))
