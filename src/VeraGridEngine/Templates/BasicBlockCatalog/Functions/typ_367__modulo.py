...
"""
Standalone EMT template for the basic catalog block 'MODULO'.
...
def build_typ_367__modulo_default_template_name() -> str:
    """
...
def build_typ_367__modulo_template(vf: VarFactory, name: str | None = None) -> EmtModelTemplate:
    """
    ...
    template_name: str
    if name is None:
        template_name = build_typ_367__modulo_default_template_name()
    else:
        template_name = name
    ...
    MODULO_n: Var = vf.add_var('MODULO__n_' + template_name)
    ...
    algebraic_equations.append((yo - (yi - (sym.floor((yi / MODULO_n)) * MODULO_n))))
    ...
    event_parameters[MODULO_n] = vf.add_const(None, name='n')
