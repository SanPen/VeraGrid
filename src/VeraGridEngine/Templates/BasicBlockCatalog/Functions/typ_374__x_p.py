...
"""
Standalone EMT template for the basic catalog block 'x^p'.
...
def build_typ_374__x_p_default_template_name() -> str:
    """
...
def build_typ_374__x_p_template(vf: VarFactory, name: str | None = None) -> EmtModelTemplate:
    """
    ...
    template_name: str
    if name is None:
        template_name = build_typ_374__x_p_default_template_name()
    else:
        template_name = name
    ...
    x_p_p: Var = vf.add_var('x^p__p_' + template_name)
    ...
    algebraic_equations.append((yo - (yi ** x_p_p)))
    ...
    event_parameters[x_p_p] = vf.add_const(None, name='p')
