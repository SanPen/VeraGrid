...
"""
Standalone EMT template for the basic catalog block 'Clarke transform'.
...
def build_typ_534__clarke_transform_default_template_name() -> str:
    """
    Return the canonical runtime name for this standalone template.

    :returns: Default template name.
    """
    return 'Clarke transform__534'

def build_typ_534__clarke_transform_template(vf: VarFactory, name: str | None = None) -> EmtModelTemplate:
    """
    ...
    template_name: str
    if name is None:
        template_name = build_typ_534__clarke_transform_default_template_name()
    else:
        template_name = name
    ...
    algebraic_equations.append((alpha - ((((sym.Const(2.0) * a) - b) - c) / sym.Const(3.0))))
    algebraic_equations.append((beta - (((sym.sqrt(sym.Const(3.0)) * b) - (sym.sqrt(sym.Const(3.0)) * c)) / sym.Const(3.0))))
    algebraic_equations.append((gamma - (((a + b) + c) / sym.Const(3.0))))
