...
"""
Standalone EMT template for the basic catalog block 'Inverse Park transform (dq)'.
...
def build_typ_537__inverse_park_transform_dq_default_template_name() -> str:
    """
    Return the canonical runtime name for this standalone template.

    :returns: Default template name.
    """
    return 'Inverse Park transform (dq)__537'

def build_typ_537__inverse_park_transform_dq_template(vf: VarFactory, name: str | None = None) -> EmtModelTemplate:
    """
    ...
    template_name: str
    if name is None:
        template_name = build_typ_537__inverse_park_transform_dq_default_template_name()
    else:
        template_name = name
