...
"""
Standalone EMT template for the basic catalog block 'Park transform (dq)'.
...
def build_typ_539__park_transform_dq_default_template_name() -> str:
    """
    Return the canonical runtime name for this standalone template.

    :returns: Default template name.
    """
    return 'Park transform (dq)__539'

def build_typ_539__park_transform_dq_template(vf: VarFactory, name: str | None = None) -> EmtModelTemplate:
    """
    ...
    template_name: str
    if name is None:
        template_name = build_typ_539__park_transform_dq_default_template_name()
    else:
        template_name = name
    ...
    algebraic_equations.append((d - ((cosphi * alpha) + (sinphi * beta))))
    algebraic_equations.append((q - (((-sinphi) * alpha) + (cosphi * beta))))
