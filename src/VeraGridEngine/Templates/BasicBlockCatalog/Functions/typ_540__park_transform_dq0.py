...
"""
Standalone EMT template for the basic catalog block 'Park transform (dq0)'.
...
def build_typ_540__park_transform_dq0_default_template_name() -> str:
    """
    Return the canonical runtime name for this standalone template.

    :returns: Default template name.
    """
    return 'Park transform (dq0)__540'

def build_typ_540__park_transform_dq0_template(vf: VarFactory, name: str | None = None) -> EmtModelTemplate:
    """
    ...
    template_name: str
    if name is None:
        template_name = build_typ_540__park_transform_dq0_default_template_name()
    else:
        template_name = name
    ...
    algebraic_equations.append((d - ((alpha * cosphi) + (beta * sinphi))))
    algebraic_equations.append((q - (((-alpha) * sinphi) + (beta * cosphi))))
    algebraic_equations.append((zero - gamma))
