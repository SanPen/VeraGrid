...
"""
Standalone EMT template for the basic catalog block 'abs -> p.u. (sig)'.
...
def build_typ_555__abs_p_u_sig_default_template_name() -> str:
    """
    Return the canonical runtime name for this standalone template.

    :returns: Default template name.
    """
    return 'abs -> p.u. (sig)__555'

def build_typ_555__abs_p_u_sig_template(vf: VarFactory, name: str | None = None) -> EmtModelTemplate:
    """
    ...
    template_name: str
    if name is None:
        template_name = build_typ_555__abs_p_u_sig_default_template_name()
    else:
        template_name = name
    ...
    algebraic_equations.append((yo - (yi / base)))
