...
"""
Standalone EMT template for the basic catalog block 'p.u. -> abs (sig)'.
...
def build_typ_559__p_u_abs_sig_default_template_name() -> str:
    """
    Return the canonical runtime name for this standalone template.

    :returns: Default template name.
    """
    return 'p.u. -> abs (sig)__559'

def build_typ_559__p_u_abs_sig_template(vf: VarFactory, name: str | None = None) -> EmtModelTemplate:
    """
    ...
    template_name: str
    if name is None:
        template_name = build_typ_559__p_u_abs_sig_default_template_name()
    else:
        template_name = name
    ...
    base: Var = vf.add_var('base_' + template_name)
