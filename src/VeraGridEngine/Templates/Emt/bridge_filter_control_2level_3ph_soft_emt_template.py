...
from VeraGridEngine.Templates.Emt.bridge_filter_2level_3ph_soft_emt_template import get_bridge_filter_2level_3ph_soft_emt_template
...
def _build_bridge_filter_current_control_block(vf: VarFactory, name: str) -> Block:
    ...
    """
    v_d: Var = vf.add_var(name=f"v_d_inner_in")
    v_q: Var = vf.add_var(name=f"v_q_inner_in")
    v_0: Var = vf.add_var(name=f"v_0_inner_in")
    i_d: Var = vf.add_var(name=f"i_d_inner_in")
    i_q: Var = vf.add_var(name=f"i_q_inner_in")
    i_0: Var = vf.add_var(name=f"i_0_inner_in")
    omega_pll: Var = vf.add_var(name=f"omega_pll_inner_in")
    omega_base: Var = vf.add_var(name=f"omega_base_inner_in")
    R_f: Var = vf.add_var(name=f"R_eq_inner_in")
    L_f: Var = vf.add_var(name=f"L_eq_inner_in")
    i_0_ref: Var = vf.add_var(name=f"i_0_ref_inner_in")
    i_d_ref: Var = vf.add_var(name=f"i_d_ref_inner_in")
    i_q_ref: Var = vf.add_var(name=f"i_q_ref_inner_in")
    i_kp: Var = vf.add_var(name=f"i_kp_inner_in")
    i_ki: Var = vf.add_var(name=f"i_ki_inner_in")
    aw_gain: Var = vf.add_var(name=f"aw_gain_inner_in")
    m_max: Var = vf.add_var(name=f"m_max_inner_in")
    Vdc_ref: Var = vf.add_var(name=f"Vdc_ref_inner_in")
    v_dc: Var = vf.add_var(name=f"v_dc_inner_in")
    vdc_floor: Var = vf.add_var(name=f"vdc_floor_inner_in")
    sbase: Var = vf.add_var(name=f"sbase_inner_in")
    P_ref: Var = vf.add_var(name=f"P_ref_inner_in")
    Q_ref: Var = vf.add_var(name=f"Q_ref_inner_in")
    P_loss0: Var = vf.add_var(name=f"P_loss0_inner_in")
    Vpk: Var = vf.add_var(name=f"Vpk_inner_in")

    xi_id: Var = vf.add_var(name=f"xi_id")
    xi_iq: Var = vf.add_var(name=f"xi_iq")
    xi_i0: Var = vf.add_var(name=f"xi_i0")
    d_xi_id: Var = vf.add_diff_var(name=f"d_xi_id", base_var=xi_id)
    d_xi_iq: Var = vf.add_diff_var(name=f"d_xi_iq", base_var=xi_iq)
    d_xi_i0: Var = vf.add_diff_var(name=f"d_xi_i0", base_var=xi_i0)

    v_pi_d_u: Var = vf.add_var(name=f"v_pi_d_u")
    v_pi_q_u: Var = vf.add_var(name=f"v_pi_q_u")
    v_pi_0_u: Var = vf.add_var(name=f"v_pi_0_u")
    v_cmd_d_u: Var = vf.add_var(name=f"v_cmd_d_u")
    v_cmd_q_u: Var = vf.add_var(name=f"v_cmd_q_u")
    v_cmd_0_u: Var = vf.add_var(name=f"v_cmd_0_u")
    k_v_conv: Var = vf.add_var(name=f"k_v_conv")
    v_lim: Var = vf.add_var(name=f"v_lim")
    v_cmd_d: Var = vf.add_var(name=f"v_cmd_d")
    v_cmd_q: Var = vf.add_var(name=f"v_cmd_q")
    v_cmd_0: Var = vf.add_var(name=f"v_cmd_0")
    ...
    return Block(
        ...
        algebraic_vars=list([v_pi_d_u, v_pi_q_u, v_pi_0_u, v_cmd_d_u, v_cmd_q_u, v_cmd_0_u, k_v_conv, v_lim, v_cmd_d, v_cmd_q, v_cmd_0]),
        init_eqs=dict([
            (xi_id, c0),
            (xi_iq, c0),
            (xi_i0, c0),
...
def _build_bridge_filter_pll_block(vf: VarFactory, name: str) -> Block:
    ...
    :return: PLL block.
    """
    v_q: Var = vf.add_var(name=f"v_q_pll_in")
    omega_base: Var = vf.add_var(name=f"omega_base_pll_in")
    pll_kp: Var = vf.add_var(name=f"pll_kp_in")
    pll_ki: Var = vf.add_var(name=f"pll_ki_in")
    phi_v: Var = vf.add_var(name=f"phi_v_pll_in")

    theta_pll: Var = vf.add_var(name=f"theta_pll")
    xi_pll: Var = vf.add_var(name=f"xi_pll")
    omega_pll: Var = vf.add_var(name=f"omega_pll")

    d_theta_pll: Var = vf.add_diff_var(name=f"d_theta_pll", base_var=theta_pll)
    d_xi_pll: Var = vf.add_diff_var(name=f"d_xi_pll", base_var=xi_pll)
...
def _build_bridge_filter_pll_input_filter_block(vf: VarFactory, name: str) -> Block:
    ...
    """
    v_q_raw: Var = vf.add_var(name=f"v_q_pll_filter_in")
    omega_sw: Var = vf.add_var(name=f"omega_sw_pll_filter_in")

    v_q_pll_f: Var = vf.add_var(name=f"v_q_pll_f")
    d_v_q_pll_f: Var = vf.add_diff_var(name=f"d_v_q_pll_f", base_var=v_q_pll_f)
...
def _build_bridge_filter_measurement_filter_block(vf: VarFactory, name: str) -> Block:
    ...
    """
    v_d_raw: Var = vf.add_var(name=f"v_d_meas_filter_in")
    v_q_raw: Var = vf.add_var(name=f"v_q_meas_filter_in")
    v_0_raw: Var = vf.add_var(name=f"v_0_meas_filter_in")
    i_d_raw: Var = vf.add_var(name=f"i_d_meas_filter_in")
    i_q_raw: Var = vf.add_var(name=f"i_q_meas_filter_in")
    i_0_raw: Var = vf.add_var(name=f"i_0_meas_filter_in")
    omega_sw: Var = vf.add_var(name=f"omega_sw_meas_filter_in")

    v_d_meas_f: Var = vf.add_var(name=f"v_d_meas_f")
    v_q_meas_f: Var = vf.add_var(name=f"v_q_meas_f")
    v_0_meas_f: Var = vf.add_var(name=f"v_0_meas_f")
    i_d_meas_f: Var = vf.add_var(name=f"i_d_meas_f")
    i_q_meas_f: Var = vf.add_var(name=f"i_q_meas_f")
    i_0_meas_f: Var = vf.add_var(name=f"i_0_meas_f")

    d_v_d_meas_f: Var = vf.add_diff_var(name=f"d_v_d_meas_f", base_var=v_d_meas_f)
    d_v_q_meas_f: Var = vf.add_diff_var(name=f"d_v_q_meas_f", base_var=v_q_meas_f)
    d_v_0_meas_f: Var = vf.add_diff_var(name=f"d_v_0_meas_f", base_var=v_0_meas_f)
    d_i_d_meas_f: Var = vf.add_diff_var(name=f"d_i_d_meas_f", base_var=i_d_meas_f)
    d_i_q_meas_f: Var = vf.add_diff_var(name=f"d_i_q_meas_f", base_var=i_q_meas_f)
    d_i_0_meas_f: Var = vf.add_diff_var(name=f"d_i_0_meas_f", base_var=i_0_meas_f)

    eps: Const = Const(1.0e-10)
    c_pi: Const = Const(np.pi)
    c0: Const = Const(0.0)
    tau_meas_expr: Expr = c_pi / sym.max(omega_sw, eps)

    return Block(
        state_eqs=list([
            (v_d_raw - v_d_meas_f) / tau_meas_expr,
            (v_q_raw - v_q_meas_f) / tau_meas_expr,
            (v_0_raw - v_0_meas_f) / tau_meas_expr,
            (i_d_raw - i_d_meas_f) / tau_meas_expr,
            (i_q_raw - i_q_meas_f) / tau_meas_expr,
            (i_0_raw - i_0_meas_f) / tau_meas_expr,
        ]),
...
def get_bridge_filter_control_2level_3ph_soft_emt_template(vf: VarFactory, name: str = "bridge_filter_control_2level_3ph_soft_emt") -> EmtModelTemplate:
    """
    Build one standalone soft-sign bridge + filter + control EMT template.
    ...
    omega_base_in: Var = vf.add_var(name=f"omega_base_in")
    v_A_in: Var = vf.add_var(name=f"v_A_in")
    v_B_in: Var = vf.add_var(name=f"v_B_in")
    v_C_in: Var = vf.add_var(name=f"v_C_in")
    v_dc_in: Var = vf.add_var(name=f"v_dc_in")
    i_d_ref_in: Var = vf.add_var(name=f"i_d_ref_in")
    i_q_ref_in: Var = vf.add_var(name=f"i_q_ref_in")
    i_0_ref_in: Var = vf.add_var(name=f"i_0_ref_in")
    m_max_in: Var = vf.add_var(name=f"m_max_in")
    vdc_floor_in: Var = vf.add_var(name=f"vdc_floor_in")
    omega_sw_in: Var = vf.add_var(name=f"omega_sw_in")
    omega_meas_in: Var = vf.add_var(name=f"omega_meas_in")
    carrier_phase_in: Var = vf.add_var(name=f"carrier_phase_in")
    carrier_enable_in: Var = vf.add_var(name=f"carrier_enable_in")
    R_f_in: Var = vf.add_var(name=f"R_f_in")
    L_f_in: Var = vf.add_var(name=f"L_f_in")
    pll_kp_in: Var = vf.add_var(name=f"pll_kp_in")
    pll_ki_in: Var = vf.add_var(name=f"pll_ki_in")
    i_kp_in: Var = vf.add_var(name=f"i_kp_in")
    i_ki_in: Var = vf.add_var(name=f"i_ki_in")
    aw_gain_in: Var = vf.add_var(name=f"aw_gain_in")
    phi_v_in: Var = vf.add_var(name=f"phi_v_in")
    Vdc_ref_in: Var = vf.add_var(name=f"Vdc_ref_in")
    sbase_in: Var = vf.add_var(name=f"sbase_in")
    P_ref_in: Var = vf.add_var(name=f"P_ref_in")
    Q_ref_in: Var = vf.add_var(name=f"Q_ref_in")
    P_loss0_in: Var = vf.add_var(name=f"P_loss0_in")
    Vpk_in: Var = vf.add_var(name=f"Vpk_in")

    plant_block: Block = get_bridge_filter_2level_3ph_soft_emt_template(vf=vf, name=f"{name}_plant").block
    ...
    meas_v_d_f: Var | None = measurement_filter_lookup.get(f"v_d_meas_f", None)
    meas_v_q_f: Var | None = measurement_filter_lookup.get(f"v_q_meas_f", None)
    meas_v_0_f: Var | None = measurement_filter_lookup.get(f"v_0_meas_f", None)
    meas_i_d_f: Var | None = measurement_filter_lookup.get(f"i_d_meas_f", None)
    meas_i_q_f: Var | None = measurement_filter_lookup.get(f"i_q_meas_f", None)
    meas_i_0_f: Var | None = measurement_filter_lookup.get(f"i_0_meas_f", None)
    pll_input_v_q_f: Var | None = pll_input_filter_lookup.get(f"v_q_pll_f", None)
    pll_theta: Var | None = pll_lookup.get(f"theta_pll", None)
    pll_omega: Var | None = pll_lookup.get(f"omega_pll", None)
    ctrl_v_cmd_d: Var | None = current_ctrl_lookup.get(f"v_cmd_d", None)
    ctrl_v_cmd_q: Var | None = current_ctrl_lookup.get(f"v_cmd_q", None)
    ctrl_v_cmd_0: Var | None = current_ctrl_lookup.get(f"v_cmd_0", None)
    ctrl_k_v_conv: Var | None = current_ctrl_lookup.get(f"k_v_conv", None)
