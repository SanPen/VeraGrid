...
def _resolve_converter_control_reference_exprs(
    ...
    # ------------------------------------------------------------------
    # Preserve the power-flow operating point only when neither controller is an
    # explicit active-power mode nor a DC-voltage mode. In Vm_dc mode the master
    # terminal must be free to create the active-power transfer required to hold
    # the DC link, rather than inheriting a frozen scheduled P from the power-flow
    # initialization.
    # ------------------------------------------------------------------
    p_ref = (
        ...
        * (
            ...
            + (Const(1.0) - (control2_is_pac + control2_is_pdc))
            * ((Const(1.0) - sym.max(control1_is_vm_dc, control2_is_vm_dc)) * p0)
        )
    )
...
def _build_pseudo_emt_converter_outer_loop_block(vf: VarFactory, name: str) -> Block:
    ...
    # The VSC power measurement uses the standard 3/2 dq scaling, so the outer
    # loop must invert that same P/Q relation to produce dimensionally
    # consistent dq current references.
    v_sq_eff = v_d * v_d + v_q * v_q + eps
    p_ctrl_pu = P_ac_ff_pu + (regulate_vdc_mode + regulate_active_mode) * (vdc_kp * active_control_error + xi_vdc)
    q_ctrl_pu = q_ref_enabled_pu + regulate_q_mode * (q_kp * (q_ref_enabled_pu - Q_f) + xi_q)
    ...
    return Block(
        ...
        algebraic_eqs=[
            ...
            # Feedforward currents are obtained from the inverse of the same
            # 3/2-scaled dq power equations used by the VSC measurement block.
            i_d_ff - c23 * (v_d * P_ac_ff_pu - v_q * q_ref_enabled_pu) / v_sq_eff,
            i_q_ff - c23 * (v_q * P_ac_ff_pu + v_d * q_ref_enabled_pu) / v_sq_eff,
            # Zero-sequence reference schedule for the balanced operating mode.
            i_0_ref_u,
            # Recover dq current references through the inverse of the same 3/2
            # P/Q equations used by the VSC measurement block.
            i_d_ref_u - c23 * (v_d * p_ctrl_pu - v_q * q_ctrl_pu) / v_sq_eff,
            # Priority-limited d-axis current reference.
            i_d_ref - i_d_cap,
            i_q_ref_u - c23 * (v_q * p_ctrl_pu + v_d * q_ctrl_pu) / v_sq_eff,
        ...
        init_eqs={
            xi_vdc: regulate_vdc_mode * (i_d0 - (c23 * P_ac_ff_pu / (Vpk + eps)) - vdc_kp * active_control_error),
