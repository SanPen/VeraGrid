...
def get_dc_line_with_power_input_emt_template(vf: VarFactory, name: str = "dc_line_power_input_emt") -> EmtModelTemplate:
    ...
    r2 = vf.add_var(name=f"R2_{name}")
    l2 = vf.add_var(name=f"L2_{name}")
    r3 = vf.add_var(name=f"R3_{name}")
    l3 = vf.add_var(name=f"L3_{name}")
    c_end_f = vf.add_var(name=f"C_end_f_{name}")
    c_mid_1 = vf.add_var(name=f"C_mid_1_{name}")
    c_mid_2 = vf.add_var(name=f"C_mid_2_{name}")
    c_end_t = vf.add_var(name=f"C_end_t_{name}")
    p_ext = vf.add_var(name=f"Pext_{name}")

    v_n1 = vf.add_var(name=f"v_n1_{name}")
    v_n2 = vf.add_var(name=f"v_n2_{name}")
    q_end_f = vf.add_var(name=f"q_end_f_{name}")
    q_mid_1 = vf.add_var(name=f"q_mid_1_{name}")
    q_mid_2 = vf.add_var(name=f"q_mid_2_{name}")
    q_end_t = vf.add_var(name=f"q_end_t_{name}")
    i_r1 = vf.add_var(name=f"i_r1_{name}")
    i_r2 = vf.add_var(name=f"i_r2_{name}")
    i_r3 = vf.add_var(name=f"i_r3_{name}")

    d_q_end_f = vf.add_diff_var(name=f"d_q_end_f_{name}", base_var=q_end_f)
    d_q_mid_1 = vf.add_diff_var(name=f"d_q_mid_1_{name}", base_var=q_mid_1)
    d_q_mid_2 = vf.add_diff_var(name=f"d_q_mid_2_{name}", base_var=q_mid_2)
    d_q_end_t = vf.add_diff_var(name=f"d_q_end_t_{name}", base_var=q_end_t)
    d_i_r1 = vf.add_diff_var(name=f"d_i_r1_{name}", base_var=i_r1)
    d_i_r2 = vf.add_diff_var(name=f"d_i_r2_{name}", base_var=i_r2)
    d_i_r3 = vf.add_diff_var(name=f"d_i_r3_{name}", base_var=i_r3)

    i_cap_end_f = vf.add_var(name=f"i_cap_end_f_{name}")
