...
"""
Maintainable international-standard module for 'Dc1c'.
...
def build_dc1c_template(vf: VarFactory, name: str | None = None) -> RmsModelTemplate:
    """
    ...
    state_equations.append(((avr_derivative_zeroGain * sym.Const(0.0)) + ((sym.Const(1.0) - avr_derivative_zeroGain) * ((avr_limIntegrator_y - avr_derivative_x) / avr_derivative_T))))
    ...
    state_equations.append(((avr_firstOrder_k - avr_firstOrder_y) / avr_firstOrder_T))
    ...
    algebraic_equations.append((avr_derivative_y - ((avr_derivative_zeroGain * sym.Const(0.0)) + ((sym.Const(1.0) - avr_derivative_zeroGain) * ((avr_derivative_k / avr_derivative_T) * (avr_limIntegrator_y - avr_derivative_x))))))
    ...
    algebraic_equations.append((avr_sum1_u_1 - (((avr_add3_k1 * avr_UsRefPu) + (avr_add3_k2 * avr_firstOrder_y)) + (avr_add3_k3 * avr_UPssPu))))
    ...
    algebraic_equations.append((avr_min1_u_3 - (((avr_transferFunction_bb_2 - (avr_transferFunction_d * avr_transferFunction_a_2)) * avr_transferFunction_x_1) + (avr_transferFunction_d * avr_feedback_y))))
    algebraic_equations.append((avr_limitedFirstOrder_Gk_y - (avr_limitedFirstOrder_Gk_k * avr_min1_u_3)))
    ...
    algebraic_equations.append((avr_sum1_u_3 - sym.Const(0.0)))
    algebraic_equations.append((avr_power_y - sym.exp((avr_BEx * avr_limIntegrator_y))))
    algebraic_equations.append((avr_add_y - ((avr_add_k1 * avr_power_y) + (avr_add_k2 * avr_const_k))))
    ...
    input_variables.append(avr_USclOelPu)
    input_variables.append(avr_USclUelPu)
    ...
    initial_equations[avr_min1_u_3] = avr_transferFunction_y_start
    initial_equations[avr_product_y] = (avr_add_y * avr_limIntegrator_y)
    ...
    initial_equations[avr_add_y] = ((avr_add_k1 * avr_power_y) + (avr_add_k2 * avr_const_k))
    initial_equations[avr_power_y] = sym.exp((avr_BEx * avr_limIntegrator_y))
    ...
    initial_equations[avr_sum1_u_1] = (((avr_add3_k1 * avr_UsRefPu) + (avr_add3_k2 * avr_firstOrder_y)) + (avr_add3_k3 * avr_UPssPu))
    ...
    template.comment = 'Generator AVR/exciter IEEE DC1C'
    return template
