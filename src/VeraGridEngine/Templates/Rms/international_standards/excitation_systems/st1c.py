...
"""
Maintainable international-standard module for 'St1c'.
...
def build_st1c_template(vf: VarFactory, name: str | None = None) -> RmsModelTemplate:
    """
    ...
    state_equations.append(((avr_derivative_zeroGain * sym.Const(0.0)) + ((sym.Const(1.0) - avr_derivative_zeroGain) * ((avr_min2_u_3 - avr_derivative_x) / avr_derivative_T))))
    ...
    state_equations.append((((avr_transferFunction1_a_end * avr_min1_u_3) - (avr_transferFunction1_a_2 * avr_transferFunction1_x_scaled_1)) / avr_transferFunction1_a_1))
    ...
    algebraic_equations.append((avr_min2_u_3 - (avr_add1_k2 * avr_feedback1_y)))
    algebraic_equations.append((avr_derivative_y - ((avr_derivative_zeroGain * sym.Const(0.0)) + ((sym.Const(1.0) - avr_derivative_zeroGain) * ((avr_derivative_k / avr_derivative_T) * (avr_min2_u_3 - avr_derivative_x))))))
    algebraic_equations.append((avr_EfdPu - ((sym.heaviside(((avr_min2_u_3 - avr_add2_y) - sym.Const(1e-06))) * avr_add2_y) + ((sym.Const(1.0) - sym.heaviside(((avr_min2_u_3 - avr_add2_y) - sym.Const(1e-06)))) * ((sym.heaviside(((avr_gain2_k - avr_min2_u_3) - sym.Const(1e-06))) * avr_gain2_k) + ((sym.Const(1.0) - sym.heaviside(((avr_gain2_k - avr_min2_u_3) - sym.Const(1e-06)))) * avr_min2_u_3))))))
    ...
    algebraic_equations.append((avr_transferFunction1_y - (((avr_transferFunction1_bb_2 - (avr_transferFunction1_d * avr_transferFunction1_a_2)) * avr_transferFunction1_x_1) + (avr_transferFunction1_d * avr_min1_u_3))))
    ...
    template.comment = 'Generator AVR/exciter IEEE ST1C'
    return template
