...
"""
Maintainable international-standard module for 'IEEEX2'.
...
def build_ieeex2_template(vf: VarFactory, name: str | None = None) -> RmsModelTemplate:
    """
    ...
    avr_tF2: Var = vf.add_var('avr.tF2_' + template_name)
    ...
    state_equations.append(((avr_derivative_zeroGain * sym.Const(0.0)) + ((sym.Const(1.0) - avr_derivative_zeroGain) * ((avr_firstOrder1_y - avr_derivative_x) / avr_derivative_T))))
    ...
    state_equations.append((((avr_firstOrder1_k * avr_limitedFirstOrder_y) - avr_firstOrder1_y) / avr_firstOrder1_T))
    ...
    algebraic_equations.append((avr_derivative_y - ((avr_derivative_zeroGain * sym.Const(0.0)) + ((sym.Const(1.0) - avr_derivative_zeroGain) * ((avr_derivative_k / avr_derivative_T) * (avr_firstOrder1_y - avr_derivative_x))))))
    ...
    event_parameters[avr_firstOrder1_T] = avr_tF2
    event_parameters[avr_firstOrder1_k] = vf.add_const(1.0, name='')
    event_parameters[avr_firstOrder1_y_start] = avr_Va0Pu
    ...
    event_parameters[avr_tF2] = vf.add_const(1.0, name='')
    ...
    template.comment = 'Generator AVR/exciter IEEEX2'
    return template
