...
def test_contingency_ts_clustering_ieee39_1w() -> None:
    """
    Verify clustered contingency time-series results match the full IEEE39_1W run
    at the sampled timestamps, and expand back to the original time axis correctly.
    """
    ...
    full_driver = ContingencyAnalysisTimeSeriesDriver(
        grid=main_circuit,
        options=options
    )
    full_driver.run()

    clustering_res = clustering(circuit=main_circuit, n_points=12)

    clustered_driver = ContingencyAnalysisTimeSeriesDriver(
        grid=main_circuit,
        options=options,
        clustering_results=clustering_res
    )
    clustered_driver.run()

    assert clustered_driver.results.nt == len(clustering_res.time_indices)
    assert len(clustered_driver.results.time_array) == len(clustering_res.time_indices)

    for it, t in enumerate(clustering_res.time_indices):
        assert np.allclose(full_driver.results.max_flows[t, :], clustered_driver.results.max_flows[it, :])
        assert np.allclose(full_driver.results.max_loading[t, :], clustered_driver.results.max_loading[it, :])
        assert np.allclose(full_driver.results.S[t, :], clustered_driver.results.S[it, :])

    clustered_driver.results.expand_clustered_results()

    assert clustered_driver.results.nt == len(clustering_res.time_indices)
    assert len(clustered_driver.results.time_array) == len(main_circuit.time_profile)
    assert clustered_driver.results.S.shape == full_driver.results.S.shape
    assert clustered_driver.results.max_flows.shape == full_driver.results.max_flows.shape
    assert clustered_driver.results.max_loading.shape == full_driver.results.max_loading.shape

    for t in clustering_res.time_indices:
        assert np.allclose(full_driver.results.S[t, :], clustered_driver.results.S[t, :])
        assert np.allclose(full_driver.results.max_flows[t, :], clustered_driver.results.max_flows[t, :])
