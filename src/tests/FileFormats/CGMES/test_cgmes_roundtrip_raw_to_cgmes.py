...
def run_raw_to_cgmes(import_path: str | list[str],
    ...
    if numerical_match:
        ...
        ybus_1 = admittance_1.Ybus.toarray()
        ybus_2 = admittance_2.Ybus.toarray()
        print("Y1")
        print(ybus_1)
        print("Y2")
        print(ybus_2)
        print("Y_diff", ybus_2 - ybus_1)
        mask = ~np.isclose(ybus_1, ybus_2, atol=1e-4, rtol=0)
        print("Y1_elements", ybus_1[mask])
        print("Y2_elements", ybus_2[mask])
        print("Y_diff", ybus_1[mask] - ybus_2[mask])
